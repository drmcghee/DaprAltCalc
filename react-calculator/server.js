...
/**
The following routes forward requests (using pipe) from our React client to our dapr-enabled services. Our Dapr sidecar lives on localhost:<daprPort>. 
...
// David: include a save method
app.post('/calculate/save', async (req, res) => {
  const saveUrl = `${daprUrl}/saveapp/method/save`;
  req.pipe(request(saveUrl)).pipe(res);
});
